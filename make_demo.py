...
const STR = {
 ja: {
  ...
  evidence:'各指標の解釈は SNA の標準的な読み方（Chen et al. 2022 のレビュー）と依存ネットワーク研究の知見（Decan et al. 2019）を踏まえている。型分類の規則自体は本システムの設計（規則表は README を参照）。',
 ...
 en: {
  ...
  evidence:'The metric interpretations draw on standard readings of SNA metrics (as synthesized in Chen et al. 2022) and findings on dependency networks (Decan et al. 2019); the type-classification rules are the design of this system. See the README for the full rule table.',
 ...
 zhHant: {
  ...
  evidence:'各指標的解讀參考 SNA 的標準讀法（整理於 Chen et al. 2022 的綜述）與依賴網路研究的成果（Decan et al. 2019）；型分類規則本身為本系統的設計（規則表見 README）。',
 ...
 zhHans: {
  ...
  evidence:'各指标的解读参考 SNA 的标准读法（整理于 Chen et al. 2022 的综述）与依赖网络研究的成果（Decan et al. 2019）；型分类规则本身为本系统的设计（规则表见 README）。',
