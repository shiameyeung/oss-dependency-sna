...
function diagnoseNode(n, g){
  ...
  let badges = types.map(t=>`<span class="badge" title="${esc(S.badgeTip[t]||"")}" style="background:${TYPE_INFO[t].color}">${TYPE_INFO[t][lang]}</span>`).join("");
  if (n.seed) badges += `<span class="badge" title="${esc(S.badgeTip.seed)}" style="background:#5DA8E8">${S.seedBadge}</span>`;
  if (n.cat) badges += `<span class="badge" title="${esc(S.badgeTip.cat)}" style="background:#8FA8C0">${esc(catLabel(n.cat))}</span>`;
  const d = descOf(n);
...
function diagnoseCom(cid, g){
  ...
  return `<div class="diag-name">${S.comTitle(cid)}</div>
    <span class="badge" title="${esc(S.badgeTip.cat)}" style="background:${COLORS[cid%COLORS.length]}; color:#1F2D40">${S.funcGroup}</span>
