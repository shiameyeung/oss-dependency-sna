...
const STR = {
 ja: {
  ...
  headerHint:'エッジの向き: 依存元 → 依存先。ノードの大きさ・色 = 選択中の指標。⚠ = 切断点（除去すると孤立が生じるノード）。ノードをクリックすると右上に診断を表示。図はホイールで拡大・ドラッグで移動できる（⟲ で初期表示に戻す）。',
 ...
 en: {
  ...
  headerHint:'Edge direction: dependent → dependency. Node size/color = selected metric. ⚠ = cut point (its removal isolates other nodes). Click a node for a diagnosis on the right. Scroll to zoom, drag to pan (⟲ resets the view).',
 ...
 zhHant: {
  ...
  headerHint:'邊的方向：依賴方 → 被依賴方。節點大小·顏色 = 所選指標。⚠ = 切斷點（移除會導致其他節點孤立）。點擊節點即在右上顯示診斷。圖可用滾輪縮放、拖曳平移（⟲ 還原初始檢視）。',
 ...
 zhHans: {
  ...
  headerHint:'边的方向：依赖方 → 被依赖方。节点大小·颜色 = 所选指标。⚠ = 切断点（移除会导致其他节点孤立）。点击节点即在右上显示诊断。图可用滚轮缩放、拖动平移（⟲ 还原初始视图）。',
